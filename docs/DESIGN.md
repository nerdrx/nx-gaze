...
**Version 1.8 · extracted from NX Hub v0.13.0 · amendments from PulseNX v1.2.1,
...
alternative — §14, 2026-09-01; compact NX wordmark — §8.1, 2026-09-09**
...
`src/renderer/styles.css` is ground truth for implemented UI components; update
this file when they change. The approved v1.8 wordmark artwork and rollout rules
in §8.1 are normative even where an existing interface has not adopted them yet.
...
| Brand wordmark | Compact lowercase **nx**, supplied as outlined SVGs (§8.1). The shared signature for headers, websites, docs, and brand lockups. |
| App icon | A beveled glass crystal hexagon (pointy-top, always) with a sculpted geometric monogram; launcher and tray variants remain in §8. |
| Type | System UI stack. No webfonts required. The wordmark is fixed vector artwork, not live UI text (§8.1). |
...
- **NX Violet is exactly `#7700FF`** for brand artwork. Accessible UI token variants do not redefine the brand color. Cyan is **very sparing and optional** — a tiny functional signal or light inside glass, never a competing brand color. The compact wordmark contains no cyan.
...
- The NX **app icon** is a **pointy-top hexagon**. Never flat-top. The compact brand wordmark is a separate, unframed signature (§8.1).
...
### 8.1 Compact NX wordmark (v1.8)

The approved compact signature is the **simple lowercase `nx`** introduced on
the NX showcase website. Use it across the NX family for mastheads, app-header
brand signatures, documentation, footers, and restrained brand lockups. Its
identity comes from heavy letterforms and the tight join between `n` and `x`.

![Compact NX wordmark in brand violet](../assets/brand/nx-wordmark-violet.svg)

**Canonical artwork:** [assets/brand](../assets/brand/README.md). These SVGs
contain outlines, so every platform gets the same silhouette without installing
or downloading a font. Copy the artwork; do not approximate it with a local font,
retype it in uppercase, or redraw the letters independently.

| Asset | Use |
| --- | --- |
| [`nx-wordmark.svg`](../assets/brand/nx-wordmark.svg) | Monochrome, `currentColor`. Inline SVG inherits the surrounding ink color; a standalone image defaults to black. |
| [`nx-wordmark-light.svg`](../assets/brand/nx-wordmark-light.svg) | Fixed soft-white `#EFEAFF` on dark grounds, including OLED. |
| [`nx-wordmark-violet.svg`](../assets/brand/nx-wordmark-violet.svg) | Fixed brand violet **`#7700FF`** on a contrasting neutral ground. |

**Construction and sizing**

- Source construction: Geist weight **900**, lowercase `nx`, **−0.125em**
  tracking (the website's −5px at 40px). This is artwork provenance, not a
  requirement to add Geist to application typography.
- The outlined artwork has a **1098 × 552** viewBox (about **1.989:1**).
  Preserve that ratio; size one dimension and let the other follow.
- Recommended visible ink height: **24–32px** in headers, **16px minimum**.
  Give the mark clear space of at least **half its visible height** on all
  sides; the tight SVG viewBox does not include that space.
- App names such as “Hub” or “Warp” sit outside that clear space in normal
  system typography. Do not attach extra letters to the `nx` silhouette.
- Use a single flat ink color. No cyan, gradients, bevels, glows, outlines,
  decorative dots, stretching, or independent letter animation inside the mark.
  A nearby status indicator is separate UI and must have its own semantics.
- Choose the supplied variant that stays legible on its background. In an
  inline monochrome mark, use the surface's normal high-contrast text token.
  Do not recolor the brand-violet asset to a theme's lighter action token.

**Accessible use**

```html
<a href="/" aria-label="NX home" class="nx-brand-link">
  <img src="/assets/brand/nx-wordmark-light.svg" alt=""
       class="nx-wordmark" />
</a>
```

```css
.nx-brand-link { display: inline-flex; padding: 12px; }
.nx-wordmark { display: block; height: 24px; width: auto; }
.nx-brand-link:focus-visible { outline: 2px solid #7700ff; outline-offset: 4px; }
```

A standalone informative image gets `alt="NX"`. If a link already has an
accessible name, use empty `alt` to avoid duplicate announcements. Inline SVG
can use `aria-hidden="true"` when its parent carries that name. An external SVG
used through `<img>` does **not** inherit the page's `color`; use a fixed-color
variant there. Allow at least a 44px target for clickable header branding.

**Rollout handoff — for the agent applying this across projects**

1. Read this guide from NX Hub, then inventory each project's visible brand
   signatures, asset pipeline, and theme support before changing them.
2. Adopt this wordmark in brand-signature placements, copying the canonical
   SVGs as needed. Keep the paths intact and record this guide's version.
3. Preserve application names, UI font stacks, navigation behavior, and layout
   clearances. Use the appropriate ink variant in each supported theme.
4. Keep the existing crystal launcher/app icons, favicons, tray icons, and
   platform icon variants unless their replacement is separately requested.
   This addition approves a shared wordmark, not an automatic icon migration.
5. Check light/dark rendering, smallest supported size, clipping, keyboard
   focus, accessible names, and responsive headers. Run GUI checks in the
   project's established headless environment (gamescope where applicable).
6. Report the projects and placements changed, plus intentional exceptions.

This v1.8 change supplies the guide and assets. Cross-project application is a
separate rollout; existing interfaces have not been migrated by this amendment.

### 8.2 NX lettering in app icons (2026-09-09)

The user-approved icon rollout extends the compact lowercase outlines to existing
app, launcher, tray, and favicon logos that contain NX lettering. Preserve each
app's recognizable silhouette and domain motif; replace the old uppercase or
font-dependent NX with the canonical paths from §8.1. Do not add NX to icons that
have no NX lettering.

Inside an app icon, center the outlines optically in the existing letter area,
using one uniform scale and flat contrasting ink. Keep ornament out of the
letterforms. Retain platform safe zones and native monochrome/themed behavior.
The 16px minimum visible height and half-height clear space in §8.1 govern
standalone signatures; compact launcher and tray compositions use their existing
icon canvas and optical spacing, verified at actual raster sizes. Keep distinct
small-size sources where needed rather than scaling detailed master art blindly.

Regenerate checked-in raster, ICO and platform derivatives from their maintained
vector sources. NX Hub's desktop outputs can be rebuilt with
`scripts/render-icons.sh` (librsvg and ImageMagick). Android foreground and
monochrome vectors retain the same canonical paths within their safe zones.
...
angular rule across. (The app icon stays the pointy-top hexagon; the compact brand signature is
the wordmark in §8.1. Icon containers and tiles round with everything else.)
